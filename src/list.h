...
#ifndef JOURNALVIEWER_LIST_H
#define JOURNALVIEWER_LIST_H
...
template <class T> class List
{
	...
	T *insert(T *before);
	// Insert new item before supplied item
	T *insertBefore(T* thisItem);
...
// Return second item in the list
...
// Insert new item before supplied item
template <class T> T *List<T>::insertBefore(T* thisItem)
{
	T *newitem = new T;
	// Get pointer to prev item before newNext (our newprev)
	T *newprev = (thisItem == NULL ? listTail_ : thisItem->prev);
	// Re-point newNext and the new item
	if (thisItem != NULL) thisItem->prev = newitem;
	else listTail_ = newitem;
...
// Return item at given position (slow)
