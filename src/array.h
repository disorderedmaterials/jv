...
#ifndef JOURNALVIEWER_ARRAY_H
#define JOURNALVIEWER_ARRAY_H
...
template <class A> class Array : public ListItem< Array<A> >
{
	...
	// Operator= (set all)
	void operator=(const double value) { for (int n=0; n<nItems_; ++n) array_[n].y() = value; }
	void operator=(const int value) { for (int n=0; n<nItems_; ++n) array_[n].y() = value; }
	// Operator+= (add to all)
	void operator+=(const double value) { for (int n=0; n<nItems_; ++n) array_[n].y() += value; }
	void operator+=(const int value) { for (int n=0; n<nItems_; ++n) array_[n].y() += value; }
